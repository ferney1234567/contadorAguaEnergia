...
/* =========================
   📄 FUNCIÓN PRINCIPAL
========================= */
export const exportarEnergiaPDF = async (registros: any[] = []) => {
  const doc = new jsPDF();
  ...
  doc.setFontSize(11);
  doc.text("REPORTE DE ENERGÍA", 105, 25, { align: "center" });
  ...
  const filas = (registros || []).map((r: any) => {
    ...
    return [
      ...
      `${r?.bombillas_c || 0}/${r?.bombillas_nc || 0}`,
      `${r?.reflectores_c || 0}/${r?.reflectores_nc || 0}`,
      `${r?.lamparas_c || 0}/${r?.lamparas_nc || 0}`,
      `${r?.aires_c || 0}/${r?.aires_nc || 0}`,
